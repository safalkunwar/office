...
async function loadMetrics() {
    ...
    try {
        ...
        const successRate = applications.length > 0
            ? (successfulApplications.length / applications.length * 100).toFixed(1)
            : 0;
...
async function generateDetailedReport() {
    ...
    // Define mapping based on header title
    const fieldMap = {
        'Name': 'name',
        'Email': 'email',
        'Course': 'course',
    ...
    reportData.forEach(row => {
        ...
        tr.innerHTML = tableHeaders.map(header => `<td>${row[fieldMap[header]] || ''}</td>`).join('');
...
async function fetchReportData(reportType) {
    try {
        const dataRef = ref(db, reportType);
...
function calculateRevenue(applications) {
    return applications.reduce((total, app) => total + (app.applicationFee || 0), 0);
}
...
window.exportReport = function () {
    ...
    alert(`Exporting ${reportType} report for last ${period} days... (To be implemented)`);
};
